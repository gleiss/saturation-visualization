...
class VampireWrapper:
	...
	def start(self, inputFile):
		...
		output = run(["/Users/bernhard/repos/vampire-release/vampire_rel_manualcl_4057", "--input_syntax", "smtlib2", "-av", "off", inputFile, "--manual_cs", "off", "--show_preprocessing", "on", "--show_new", "on", "--show_passive", "on", "--show_active", "on", "--show_reductions", "on", "-t", "3"], stdout=PIPE, stderr=STDOUT, text=True).stdout
	...
	def startManualCS(self, inputFile):
		if self.vampireProcess != None:
			self.vampireProcess.kill()
		self.vampireProcess = Popen(["/Users/bernhard/repos/vampire-release/vampire_rel_manualcl_4057", "--input_syntax", "smtlib2", "-av", "off", inputFile, "--manual_cs", "on", "--show_preprocessing", "on", "--show_new", "on", "--show_passive", "on", "--show_active", "on", "--show_reductions", "on"], stdin=PIPE, stdout=PIPE, stderr=STDOUT)
